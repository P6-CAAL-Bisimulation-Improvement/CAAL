...
QUnit.test("Simple not bisimilar", function (assert) {
    var graph = CCSParser.parse("P = a.(b.0 + c.0); Q = a.b.0 + a.c.0;", {
            ccs: CCS,
        }),
...
QUnit.test("Bisimilarity of Recursive Processes", function (assert) {
    var graph = CCSParser.parse("P = a.(a.0 | P);\n" + "Q = P | P;", {
        ccs: CCS,
    });
    var strongSuccGen = getStrictSuccGenerator(graph),n
        processP = graph.processByName("P"),
        processQ = graph.processByName("Q");
    assert.ok(
        Equivalence.isBisimilar(
        ...
        "P = a.(a.0|P) and Q = P|P should be bisimilar",
    );
});

QUnit.test("Simple bisimilar", function (assert) {
    var graph = CCSParser.parse(
            "P = a.(b.0 + c.0); Q = a.(b.0 + c.0) + a.(c.0 + b.0);",
            { ccs: CCS },
        ),
        strongSuccGen = getStrictSuccGenerator(graph),
        processP = graph.processByName("P"),
